...
<head>
  ...
  <title>Associar Aluno</title>

  <!-- Custom fonts for this template-->
  <link href="{{URL::asset('tema/vendor/fontawesome-free/css/all.min.css')}}" rel="stylesheet" type="text/css">
  <link href="https://fonts.googleapis.com/css?family=Nunito:200,200i,300,300i,400,400i,600,600i,700,700i,800,800i,900,900i" rel="stylesheet">
  <!-- Custom styles for this template-->
  <link href="{{URL::asset('tema/css/sb-admin-2.min.css')}}" rel="stylesheet">
      ...
      <div id="content">
        ...
        <div class="container-fluid">
          ...
          <div class="d-sm-flex align-items-center justify-content-between mb-4">
            <h1 class="h3 mb-0 text-gray-800">Selecionar Projetos</h1>
          </div>

          <!-- List Project Modal-->
          <div class="container-fluid">
            <!-- Collapse with list projects-->
            <br>
            <div class="card-body">
              <div class="accordion" id="accordionExample">
                <!--Preenchendo dinamicamente-->
                @php
                  $professores = $profProject["professores"];
                  $projeto = $profProject["projeto"];
                  $nextId = 0;
                @endphp
                @if(!empty($professores))
                  @foreach($professores as $professor)
                    ...
                    @endphp
                    <div class="card">
                      <div class="card-header" id="heading<?= $nextId?>" style="background-color:#ADD8E6">
                        <h5 class="mb-0">
                          <button class="btn btn-link" type="button" data-toggle="collapse" data-target="#collapse<?= $nextId?>" aria-expanded="true" aria-controls="collapse1">{{$professor->nome}} </button>
                        </h5>
                      </div>
                      @php
                        $projProf = $projeto[$professor->professor_orientador_id];
                      @endphp
                      @foreach($projProf as $x)
                        <div id="del"></div>
                        <div id="collapse<?= $nextId?>" class="collapse" aria-labelledby="heading<?= $nextId?>" data-parent="#accordionExample" style="">
                          <div class="card-body">Título: {{ $x->titulo_projeto }} <br> Curso: {{$x->curso_id}} <br> Área: {{$x->area}}<br> Semestre:  {{$x->semestre}}<br> Ano: {{$x->ano}}<br> Grupo de Pesquisa: {{$x->grupo_pesquisa}} <br> Descrição: {{$x->descricao}} <br><br>
                            <form action="{{route('solicitacao.store')}}" method="post">
                              {{ csrf_field() }}
                              <input type="text" class="form-control " style="display:none" name="status" id="status_soli" value="pendente">
                              <input type="text" class="form-control " style="display:none" name="aluno_id" id="idAluno_soli" {{ Auth::user()->atuacao_id == 6 ? $v=Auth::user()->id : $v=0 }}value=$v>
                              <input type="text" class="form-control " style="display:none" name="projeto_id" id="idProjeto_soli" value="{{$x->id}}">
                              <input type="text" class="form-control " style="display:none" name="curso_id" id="idCurso_soli" value="{{$x->curso_id}}">
                              <div class="bd-button">
                                @if(Auth::user()->atuacao_id == 1)
                                  <button type="submit" class="btn btn-outline-success" id="btnCandidatar" data-toggle="modal" data-target="#exampleModal">Candidatar-se</button>
                                  <button type="button" class="btn btn-outline-primary" id="btnAssociarAluno" onclick="javascript: location.href= '{{ route('associar_aluno',$x->id) }}';">Associar aluno</button>
                                @endif
          ...
          @if (session('message'))
            ...
            <script>
              $(document).ready(function(){
                $("#confirmCandidatar").modal();
              });
            </script>
            <?  ?>
            <div class="modal fade" id="confirmCandidatar" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
              <div class="modal-dialog" role="document">
                <div class="modal-content">
                  <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel">Candidar ao Projeto</h5>
                  ...
                  <div class="modal-body">
                    @php
                      $resposta = session('message');
                    @endphp
                    @if(strcmp($resposta,'ok')==0)
                      <h5>Você acabou de se candidatar a esse projeto!</h5>
                    @elseif(strcmp($resposta,'error')!=0)
                      <h5>Algo deu erro! Tente novamente.</h5>
                    @endif
                  </div>
