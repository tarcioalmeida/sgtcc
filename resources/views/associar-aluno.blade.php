    ...
    <div id="content-wrapper" class="d-flex flex-column">
        ...
        <div id="content">
            ...
            <div class="container-fluid">
                ...
                @php
                    $listAssociar;
                    if(!empty($listAssociar)){
                        $projetos = $listAssociar["projeto"];
                        $solicitacoes = $listAssociar["solicitacoes"];
                        $projeto = $projetos[0];
                        $alunos = $listAssociar["alunos"];
                    }
                @endphp
                <!-- Content Column -->
                    <div class="col-lg-6 mb-4">
                        <!-- Candidatos para associar ao projeto -->
                        <div class="card shadow mb-4">
                            <div class="card-header py-3">
                                <h6 class="m-0 font-weight-bold text-primary">Candidatos</h6>
                            </div>
                            <div class="card-body">
                                <div class="list-group" id="listCandidato">
                                    <!--Listando candidatos dinamicamente-->
                                    @if(!empty($solicitacoes)&& count($solicitacoes)>0)
                                        @foreach($solicitacoes as $solicitacao)
                                            <button href="#" onclick="escolheAluno(this.value);"  value="{{ $solicitacao->aluno_id."-".$alunos[$solicitacao->aluno_id][0]->nome }}"class="list-group-item list-group-item-action">{{ $alunos[$solicitacao->aluno_id][0]->nome }}</button>
                                        @endforeach
                                    @endif
                                </div>
                            </div>
                    ...
                    <div class="col-lg-6 mb-4">
                        ...
                        <div class="card shadow mb-4">
                            ...
                            @if(!empty($projeto) && !empty($solicitacao))
                                <div  class="card-body">
                                    <h6> Título: {{ $projeto->titulo_projeto }}</h6>
                                    <h6>Curso: {{ $projeto->curso_id }}</h6>
                                    <h6>Área: {{ $projeto->area }}</h6>
                                    <h6>Semestre: {{$projeto->semestre}}</h6>
                                    <h6>Ano: {{ $projeto->ano }}</h6>
                                    <h6>Grupo de Pesquisa: {{ $projeto->grupo_pesquisa }}</h6>
                                    <h6>Descrição: {{ $projeto->descricao }}</h6>
                                    <br>
                                    <h5 id="nomeAluno"></h5>
                                    <br>
                                    <input id="idAluno" name="aluno_id" style="display:none" value="">
                                    <div class="bd-button" align="center">

                                        <a href="{{ url('/inserir-aluno-projeto',[$projeto->id,$solicitacao->aluno_id]) }}" class="btn btn-success btn-icon-split"> <!-- Preciso ver como salvar os dados e passar por aqui | Depos ver como encapsular-->
                                            <span class="icon text-white-50">
                                              <i class="fas fa-check"></i>
                                            </span>
                                            <span class="text">Associar</span>
                                        </a>

                                        </button>
                                        <button type="button" class="btn btn-danger btn-icon-split" onclick="javascript: location.href='{{ route('index') }}'">
                                            <span class="icon text-white-50">
...
<script src="../tema/js/itemList.js"></script>

<script>
    function escolheAluno(idNomeAluno) {

        $aluno = idNomeAluno;
        var nome = $aluno.match(/[^-]+$/);
        var id = $aluno.match(/[^-]/);
        $('#nomeAluno').text( "Orientando: "+nome );
        $("#idAluno").val(id);
    }
</script>
</body>
